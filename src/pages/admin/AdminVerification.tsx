...
interface OrderSummary {
  ...
  paymentStatus: "waiting" | "verified" | "rejected";
...
export default function AdminVerification() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      setOrders(data);
      if (data.length > 0) setActiveOrder(data[0]);
    } catch (error) {
  ...
  // ฟังก์ชันอนุมัติ/ปฏิเสธ (API ใหม่)
  const handleVerifyPayment = async (status: "verified" | "rejected") => {
    if (!activeOrder) return;
    try {
      const response = await fetch(
        `http://localhost:5000/orders/admin/verify-payment/${activeOrder.id}`,
        {
          method: "PATCH",
          headers: {
      ...
      if (response.ok) {
        fetchOrders(); // ดึงข้อมูลใหม่หลังจากอัปเดตสถานะ
      }
    } catch (error) {
  ...
  if (!activeOrder) return <div className="p-10">Loading...</div>;
      ...
      <aside className="w-64 bg-[#0F172A] text-white flex flex-col p-6 relative z-20">
        ...
        <nav className="flex-1 space-y-1">
          ...
          <SidebarItem
            ...
            onClick={() => navigate("/adminproductapproval")}
          />
          <SidebarItem
            label="รายชื่อผู้ใช้"
            icon={<Users size={18} />}
                    ...
                    {orders.map((order) => (
                      ...
                      >
                        <td className="px-8 py-5 relative">
                          ...
                          <p className="text-[9px] font-bold text-slate-300 mt-0.5 uppercase tracking-tighter">
                        ...
                        <td className="px-8 py-5 text-center">
                          <span
                            className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-tighter border ${
              ...
              <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 relative overflow-hidden w-full shadow-md shadow-black/[0.03]">
                ...
                <div className="relative z-10 text-center">
                  <div className="aspect-[3/4] bg-slate-50 rounded-3xl mb-6 overflow-hidden border-4 border-white shadow-inner mx-auto max-w-[260px] group">
                    <img
                      src={`http://localhost:5000/uploads/slips/${activeOrder.paymentSlipUrl}`}
                  ...
                  <div className="space-y-4 mb-6 px-1">
                    <div className="flex justify-between items-end">
                      <p className="text-[9px] font-black text-slate-300 uppercase tracking-widest text-left">
                        Transfer Amount
                      </p>
                      <p className="text-2xl font-black text-[#0F172A] tracking-tighter">
                        ฿{parseFloat(activeOrder.totalAmount).toLocaleString()}
                      </p>
                    </div>
                    <div className="h-px bg-slate-50 w-full" />
                    <div className="grid grid-cols-2 gap-3 text-[9px] font-black uppercase text-left">
                      <div>
                        <p className="text-slate-300 mb-0.5 tracking-widest">
                          Order ID
                        </p>
                  ...
                  <div className="flex gap-3">
                    <button
                      ...
                      className="flex-1 bg-[#0F172A] text-white py-4 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-emerald-600 transition-all flex items-center justify-center gap-2 active:scale-95 shadow-md shadow-black/10"
                    >
                      <CheckCircle size={14} /> อนุมัติ
                    </button>
                    <button
                      onClick={() => handleVerifyPayment("rejected")}
                      className="px-5 bg-slate-50 text-slate-400 py-4 rounded-xl font-black text-[10px] uppercase hover:bg-red-50 hover:text-red-500 transition-all active:scale-95 border border-slate-100"
                    >
                      <XCircle size={16} />
                    </button>
                  </div>
