...
export default function Orders() {
    ...
    const fetchOrders = async () => {
      ...
      try {
        ...
        const mappedOrders: Order[] = res.data.map((order: any) => ({
          ...
          address: `${order.address} ${order.sub_district} ${order.district} ${order.province} ${order.zip_code}`,
          phone: order.phone,
  ...
  const getStatusInfo = (status: string) => {
    switch (status) {
      ...
      default:
        return { label: status, color: "bg-slate-100", icon: null };
    }
  };
      ...
      <main className="flex-grow max-w-5xl mx-auto w-full px-6 py-12">
        <header className="mb-10">
          ...
          <p className="text-slate-500 text-sm font-medium">
            จัดการประวัติการสั่งซื้อและดูรายละเอียดสินค้า
          </p>
        </header>
                ...
                >
                  <div className="px-8 py-5 bg-slate-50/50 border-b border-slate-50 flex justify-between items-center">
                    <div className="flex items-center gap-4">
                      ...
                      <div
                        className={`flex items-center gap-2 px-3 py-1 rounded-full border text-[10px] font-black uppercase ${statusInfo.color}`}
                      >
                    ...
                    {order.items.map((item) => (
                      <div key={item.id} className="flex gap-6 items-center">
                        ...
                        <div className="flex-grow">
                          <h4 className="font-bold text-[#0F172A] text-sm leading-tight">
                            {item.name}
                          </h4>
